...
namespace quda
{
  ...
  template <typename Arg> class BoostedLaplace : public Dslash<boostedLaplace, Arg>
  {
    using Dslash = Dslash<boostedLaplace, Arg>;
    using Dslash::arg;
    using Dslash::halo;
    using Dslash::in;

  public:
    BoostedLaplace(Arg &arg, cvector_ref<ColorSpinorField> &out, cvector_ref<const ColorSpinorField> &in,
            const ColorSpinorField &halo) :
      Dslash(arg, out, in, halo)
    {
    }
    ...
    { // add laplace transverse dir to the key
      auto key = Dslash::tuneKey();
      strcat(key.aux, ",boostedLaplace=");
      u32toa(key.aux + strlen(key.aux), arg.dir);
      return key;
    }
  };

  template <typename Float, int nColor, QudaReconstructType recon> struct BoostedLaplaceApply {

    BoostedLaplaceApply(cvector_ref<ColorSpinorField> &out, cvector_ref<const ColorSpinorField> &in,
                 cvector_ref<const ColorSpinorField> &x, const GaugeField &U, int dir, double a, double b, double smear_mtm_x, double smear_mtm_y, double smear_mtm_z, int parity,
                 const int *comm_override, TimeProfile &profile)
    {
      constexpr int nDim = 4;
      auto halo = ColorSpinorField::create_comms_batch(in, 1, false);
      if (in.Nspin() == 1) {
        constexpr int nSpin = 1;
        BoostedLaplaceArg<Float, nSpin, nColor, nDim, recon> arg(out, in, halo, U, dir, a, b, smear_mtm_x, smear_mtm_z, smear_mtm_z, x, parity, comm_override);
        BoostedLaplace<decltype(arg)> boostedLaplace(arg, out, in, halo);
        dslash::DslashPolicyTune<decltype(boostedLaplace)> policy(boostedLaplace, in, halo, profile);
      } else if (in.Nspin() == 4) {
        constexpr int nSpin = 4;
        BoostedLaplaceArg<Float, nSpin, nColor, nDim, recon> arg(out, in, halo, U, dir, a, b, smear_mtm_x, smear_mtm_y, smear_mtm_z, x, parity, comm_override);
        BoostedLaplace<decltype(arg)> boostedLaplace(arg, out, in, halo);
        dslash::DslashPolicyTune<decltype(boostedLaplace)> policy(boostedLaplace, in, halo, profile);
      } else {
        errorQuda("Unsupported nSpin= %d", in.Nspin());
  ...
  {
    if constexpr (is_enabled<QUDA_LAPLACE_DSLASH>()) {
      instantiate<BoostedLaplaceApply>(out, in, x, U, dir, a, b, smear_mtm_x, smear_mtm_y, smear_mtm_z, parity, comm_override, profile);
    } else {
      errorQuda("Laplace operator has not been enabled");
    }
  }
