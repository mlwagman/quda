...
#pragma unroll
  ...
  template <int nParity, bool, bool xpay, KernelType kernel_type, typename Arg> struct laplace : dslash_default {
    ...
    __device__ __host__ __forceinline__ void operator()(int idx, int src_idx_, int parity)
    {
      int src_idx = MAX_MULTI_RHS == 1 ? 0 : src_idx_;
  ...
  */
  template <typename Float, int nSpin_, int nColor_, int nDim, QudaReconstructType reconstruct_>
  struct BoostedLaplaceArg : DslashArg<Float, nDim> {
    static constexpr int nColor = nColor_;
    static constexpr int nSpin = nSpin_;
    static constexpr bool spin_project = false;
    static constexpr bool spinor_direct_load = false; // false means texture load
    ...
    const real a; /** xpay scale factor - can be -kappa or -kappa^2 */
    const real b; /** used by Wuppetal smearing kernel */
    const real smear_mtm_x; /** used by Wuppetal smearing kernel */
    const real smear_mtm_y; /** used by Wuppetal smearing kernel */
    const real smear_mtm_z; /** used by Wuppetal smearing kernel */
    int dir;      /** The direction from which to omit the derivative */

    BoostedLaplaceArg(cvector_ref<ColorSpinorField> &out, cvector_ref<const ColorSpinorField> &in,
               const ColorSpinorField &halo, const GaugeField &U, int dir, double a, double b, double smear_mtm_x, double smear_mtm_y, double smear_mtm_z,
               cvector_ref<const ColorSpinorField> &x, int parity, const int *comm_override) :
      DslashArg<Float, nDim>(out, in, halo, U, x, parity, false, a != 0.0 ? true : false, 1, false, comm_override),
      halo_pack(halo),
      halo(halo),
      ...
      a(a),
      b(b),
      smear_mtm_x(smear_mtm_x),
      smear_mtm_y(smear_mtm_y),
      smear_mtm_z(smear_mtm_z),
      dir(dir)
    {
      for (auto i = 0u; i < out.size(); i++) {
        this->out[i] = out[i];
  ...
  */
  template <int nParity, KernelType kernel_type, int dir, typename Coord, typename Arg, typename Vector>
  __device__ __host__ inline void applyBoostedLaplace(Vector &out, Arg &arg, Coord &coord, int parity, int, int thread_dim,
                                               bool &active, int src_idx)
  {
    typedef typename mapper<typename Arg::Float>::type real;
    typedef Matrix<complex<real>, Arg::nColor> Link;
    ...
    const real smear_mtm[3]  = {arg.smear_mtm_x, arg.smear_mtm_y, arg.smear_mtm_z};

#pragma unroll
    for (int d = 0; d < Arg::nDim; d++) { // loop over dimension
      if (d != dir) {
        {
          // Momentum smearing coefficients
          const real mtm_dot_pos = smear_mtm[d];
          const complex<real> fwd_coeff (cos(mtm_dot_pos), sin(mtm_dot_pos));
          // Forward gather - compute fwd offset for vector fetch
          const bool ghost = (coord[d] + 1 >= arg.dim[d]) && isActive<kernel_type>(active, thread_dim, d, coord, arg);
        ...
        }
        {
          // Momentum smearing coefficients
          const real mtm_dot_pos = smear_mtm[d];
          const complex<real> bwd_coeff (cos(mtm_dot_pos), -1*sin(mtm_dot_pos));
          // Backward gather - compute back offset for spinor and gauge fetch
  ...
  // out(x) = M*in
  template <int nParity, bool dagger, bool xpay, KernelType kernel_type, typename Arg> struct boostedLaplace : dslash_default {

    const Arg &arg;
    constexpr boostedLaplace(const Arg &arg) : arg(arg) {}
    static constexpr const char *filename() { return KERNEL_FILE; } // this file name - used for run-time compilation
    ...
    __device__ __host__ __forceinline__ void operator()(int idx, int src_idx_, int parity)
    {
      ...
      // case 3 is a spatial operator only, the t dimension is omitted.
      switch (arg.dir) {
      case 3: applyBoostedLaplace<nParity, mykernel_type, 3>(out, arg, coord, parity, idx, thread_dim, active, src_idx); break;
      case 4:
      default:
        applyBoostedLaplace<nParity, mykernel_type, -1>(out, arg, coord, parity, idx, thread_dim, active, src_idx);
        break;
      }
